...
disp('*** BRAZO DERECHO - SIMULACIÓN DE TRAYECTORIA + CLIENTE  ***');
...
% OBJETIVO: Enviar las trayectorias generadas a otro Script que realiza
%           la cinematica inversa del brazo robótico
% FECHA:    24 de Junio 2021
% DISEÑO:   Ing. Cristian Vallejo

%% Inicialización de Protocolo UDP

% u = udp('192.168.1.128',20777); % IP para enviar a server 
u = udp('127.0.0.1',20777); % IP para enviar a server 
u.InputBufferSize = 65536;
u.OutputBufferSize = 65536;
...
for i = 1:1:length(t)
    ...
    Datagram = [F1 F2 F3 theta3];
    Datagram = mat2str(Datagram);
    fprintf(u,Datagram); 
    
    % FullData = [FullData; Datagram];
    % writematrix(FullData,'Prueba.csv') 
end 
